...
const Card = (props) => {
  ...
  <div id={slugify(frontmatter.name)} className={classNames(styles.card, frontmatter.blur && styles.blur)}>
    ...
    {frontmatter.smallImage && (<div className={styles.profilePic}>
      <Img className={styles.profilePic} resolutions={frontmatter.smallImage.childImageSharp.resolutions} />
    </div>)}
    {html && <div className={styles.text} dangerouslySetInnerHTML={{__html: html}} />}
